...
3. As the current version of `create-react-app` installs React 16 while the GoodData UI SDK requires React 15.6.2, downgrade your version of React to 15.6.2 by running the following command:
...
This command adds latest `@gooddata/react-components` to the list of your project dependencies in `package.json` and downloads all required packages.
...
1. Make sure that you are in the root folder of your app, run the following command and fill the interactive prompts:
